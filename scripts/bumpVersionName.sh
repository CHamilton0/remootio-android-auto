...
if [ -e "$version_file" ]; then
    ...
    if git log --oneline --no-merges $(git rev-list --merges --max-count=2 origin/master)..origin/master --grep='BREAKING CHANGE\|fix\|feat' | grep -q 'BREAKING CHANGE\|feat\|fix'; then
        ...
        if git log --oneline --no-merges $(git rev-list --merges --max-count=2 origin/master)..origin/master --grep='BREAKING CHANGE' | grep -q 'BREAKING CHANGE'; then
            # Increment major version for breaking changes
            ((major_version++))
            minor_version=0
            patch_version=0
        elif git log --oneline --no-merges $(git rev-list --merges --max-count=2 origin/master)..origin/master --grep='feat' | grep -q 'feat'; then
